...
export class MagicEdenAPI implements NonFungibleTokenAPI.Provider<ChainId, SchemaType> {
    ...
    async getHistory(address: string, tokenId: string, { page = 1, size = 50 }: HubOptions<ChainId> = {}) {
    ...
    async getOrders(_: string, mintAddress: string, side: OrderSide, { page = 1, size }: HubOptions<ChainId> = {}) {
        const limit = size || 20
        const offset = page ? (Math.max(1, page) - 1) * limit : 0
    ...
    async getCollections(symbol: string, { page = 1, size }: HubOptions<ChainId> = {}) {
